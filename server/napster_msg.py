...
def compose_search_response(tam: int):
    reply = NapsterMsg()
    reply.type = 0x0065
    reply.payload = str(tam)
    reply.length = len(reply.payload[0])
    return reply

def compose_search_content(results: str):
    reply = NapsterMsg()
    reply.type = 0x0065
...
def compose_notification_ack() -> NapsterMsg:
    reply = NapsterMsg()
    reply.type = 0x0065
    reply.payload = 'successful notification'
    reply.length = len(reply.payload[0])
    return reply

def compose_serch_error() -> NapsterMsg:
    reply = NapsterMsg()
    reply.type = 0x0065
    reply.payload = ''
    reply.length = 0
