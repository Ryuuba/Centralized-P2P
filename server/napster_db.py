...
class DBNapsterConnector:
    ...
    def search_content(self, kwords: str) -> list[str]:
        cursor = self.__conn.cursor()
        cursor.execute('SELECT tblContent.distro,tblContent.size, tblContent.version, tblContent.arch, tblContent.target, tblUserContent.url, tblUserNetworkData.ip, tblUserNetworkData.port'
                    ...
                    + ' INNER JOIN tblUserNetworkData ON tblUser.nickname = tblUserNetworkData.nickname WHERE ? IN (tblContent.distro, tblContent.version,tblContent.size, tblContent.arch, tblContent.target)', (kwords[0],))
        results = cursor.fetchall()
        #print(tabulate(results, headers=["Distro", "Version", "Arch", "Size", "Target", "Name", "IP", "Puerto"]))
        #print(results)
        return results
        #print(tabulate(results, headers=["Distro", "Version", "Arch", "Size", "Target", "Name", "IP", "Puerto"]))
