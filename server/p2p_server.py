...
class P2PServer:
    ...
    ip: str
    port: int
    #El uso de funciones de fábrica predeterminadas es una forma de crear nuevas instancias de tipos mutables como valores predeterminados para los campos:
    ...
    def __respond_pub_key_req(self, client_socket: socket.socket):
        ...
        print("----Server Response send (key)-----")
        print(reply)
    ...
    def __record_client_content(self, client_socket: socket.socket, 
        ...
        reply = napster_msg.compose_notification_ack()
        client_socket.sendall(reply.to_bytes())
    ...
    def __respond_serch(self, client_socket: socket.socket, recv_msg: napster_msg.NapsterMsg):
        serchResponse = self.__db_conn.search_content(recv_msg.payload)
        print(f' keyword received for search request {recv_msg.payload}')
        if serchResponse:
            consulta = ''
            nCont = len(serchResponse)
            i=0
            reply = napster_msg.compose_search_response(nCont)
            client_socket.sendall(reply.to_bytes())
            
            for result in serchResponse:
                if(i<nCont):
                    for word in result:
                        consulta += str(word) + ' '
                        #print(consulta)
                    print(consulta)
                    reply = napster_msg.compose_search_content(consulta)
                    client_socket.sendall(reply.to_bytes())
                    consulta = ''
                    i=+1
            ...
            print("-------------Error---------------")
            reply = napster_msg.compose_serch_error()
            client_socket.sendall(reply.to_bytes())
    ...
    def __respond_request(self, client_socket: socket.socket):
        ...
        if recv_msg.type == 0x0010: # public key req
            ...
            self.__respond_serch(client_socket, recv_msg)
